...
export interface UcQrLoginResult {
  cookie: string;
  cookieNames: string[];
  wroteEnv: boolean;
}
...
export async function loginUcByQrCode(
  options: UcQrLoginOptions = {},
): Promise<UcQrLoginResult> {
  ...
  if (options.writeEnv) {
    ...
    writeFileSync(
      envPath,
      updateEnvValue(existing, "NETDISK_TRANSFER_UC_COOKIE", cookie),
    );
  }
